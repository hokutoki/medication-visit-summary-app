...
iPhone Safariでホーム画面に追加して使うことを想定した、服薬管理・受診サマリー用のPWAです。4週間ごとの通院に向けて、服薬状況、体調、やる気・動ける度、睡眠、心拍数、歩数、診察メモを端末内に保存し、診察時に見せる要約を作成します。
...
- 今日の体調: 良い、普通、悪い
- やる気・動ける度: 0〜10
- HealthKit相当データの手入力: 睡眠時間、安静時心拍数、平均心拍数、歩数
- 前回受診日から次回診察日の前日までの4週間サマリー
- KPI表示: 服薬達成率、平均やる気・動ける度、低活動日数、体調不良日数、平均睡眠、短時間睡眠日数、平均安静時心拍数、平均歩数、メモ日数
- Rechartsによる推移グラフ
- 医師に見せる診察用サマリー
- 気になった日の自動抽出
...
iPhone Safari上のWebアプリからHealthKitを直接読み取ることはできません。睡眠時間、安静時心拍数、平均心拍数、歩数は、初期版では手入力またはJSONインポートで扱います。

HealthKitとの直接連携が必要な場合は、将来的にSwift/HealthKitを使ったネイティブiOSアプリ、またはネイティブアプリからこのWebアプリ形式に合わせたJSONを書き出す連携が必要です。

## HealthKit JSONインポート形式

設定タブの `HealthKit JSONインポート` では、Swift/HealthKit版やショートカット等で作成した日別ヘルスデータだけを既存記録へマージできます。服薬、体調、やる気・動ける度、メモは上書きしません。

サンプル:

```json
{
  "source": "healthkit-json",
  "version": 1,
  "records": [
    {
      "date": "2026-05-08",
...
未入力・取得不可の値は `null` にしてください。取り込み後も、集計では `null` を0として扱いません。

サンプルファイルは [public/examples/healthkit-import-sample.json](public/examples/healthkit-import-sample.json) にあります。
